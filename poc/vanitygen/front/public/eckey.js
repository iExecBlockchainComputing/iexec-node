const KeyPool = (function () {
  const KeyPool = function () {
    ...
    this.push = function (item) {
      ...
      for (const index in this.keyArray) {
        const currentItem = this.keyArray[index];
        if (
          currentItem != null &&
    ...
    this.toString = function () {
      let keyPoolString = `# = ${this.length()}\n`;
      const pool = this.getArray();
      for (const index in pool) {
        const item = pool[index];
        if (Util.hasMethods(item, 'getBitcoinAddress', 'toString')) {
          if (item != null) {
...
//   const Bip38 = function (address, encryptedKey) {
...
const ECKey = (function () {
  ...
  var ECKey = function (input) {
    ...
    try {
      ...
      const hexKeyRangeLimit = 'FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364140';
      const rangeLimitBytes = Crypto.util.hexToBytes(hexKeyRangeLimit);
      const limitBigInt = BigInteger.fromByteArrayUnsigned(rangeLimitBytes);
  ...
  ECKey.prototype.getBitcoinWalletImportFormat = function () {
    ...
    const checksum = Crypto.SHA256(Crypto.SHA256(bytes, { asBytes: true }), { asBytes: true });
    bytes = bytes.concat(checksum.slice(0, 4));
    const privWif = Base58.encode(bytes);
    return privWif;
  };
  ...
  ECKey.prototype.getBitcoinPrivateKeyByteArray = function () {
    ...
    const bytes = this.priv.toByteArrayUnsigned();
  ...
  ECKey.decodeWalletImportFormat = function (privStr) {
    const bytes = Base58.decode(privStr);

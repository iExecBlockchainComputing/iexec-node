...
(function () {
  ...
  const ec = (window.EllipticCurve = function () {});
  ...
  ec.FieldElementFp.prototype.sqrt = function () {
    ...
    // p mod 4 == 1
    const qMinusOne = this.q.subtract(BigInteger.ONE);
    const legendreExponent = qMinusOne.shiftRight(1);
    if (!this.x.modPow(legendreExponent, this.q).equals(BigInteger.ONE)) return null;
    const u = qMinusOne.shiftRight(2);
    const k = u.shiftLeft(1).add(BigInteger.ONE);
    const Q = this.x;
    const fourQ = Q.shiftLeft(2).mod(this.q);
    let U,
      V;
    ...
    do {
      ...
      const result = ec.FieldElementFp.fastLucasSequence(this.q, P, Q, k);
  ...
  ec.PointFp.prototype.getX = function () {
    ...
    const r = this.x.toBigInteger().multiply(this.zinv);
    this.curve.reduce(r);
  ...
  ec.PointFp.prototype.getY = function () {
    ...
    const r = this.y.toBigInteger().multiply(this.zinv);
    this.curve.reduce(r);
    return this.curve.fromBigInteger(r);
  };
  ...
  ec.PointFp.prototype.getEncoded = function (compressed) {
    ...
    const len = 32; // integerToBytes will zero pad if integer is less than 32 bytes. 32 bytes length is required by the Bitcoin protocol.
    let enc = ec.integerToBytes(x, len);
  ...
  ec.PointFp.prototype.isOnCurve = function () {
    ...
    const rhs = x.multiply(x).multiply(x).add(a.multiply(x)).add(b).mod(n);
  ...
  ec.CurveFp.prototype.decodePointHex = function (s) {
    const firstByte = parseInt(s.substr(0, 2), 16);
  ...
  ec.CurveFp.prototype.decompressPoint = function (yTilde, X1) {
    ...
    const alpha = x.multiply(x.square().add(this.getA())).add(this.getB());
    let beta = alpha.sqrt();
    // if we can't find a sqrt we haven't got a point on the curve - run!
    if (beta == null) throw new Error('Invalid point compression');
    const betaValue = beta.toBigInteger();
    const bit0 = betaValue.testBit(0) ? 1 : 0;
    if (bit0 != yTilde) {
      // Use the other root
  ...
  ec.secNamedCurves = {
    ...
    secp256k1() {
      ...
      const p = ec.fromHex('FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC2F');
      const a = BigInteger.ZERO;
      const b = ec.fromHex('7');
      const n = ec.fromHex('FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141');
      const h = BigInteger.ONE;
      const curve = new ec.CurveFp(p, a, b);
      const G = curve.decodePointHex(
        '04' +

...
const ECKey = () => {
  ...
  const ECKey = input => {
    ...
    this.compressed = this.compressed === undefined
  ...
  ECKey.prototype.setError = err => {
    this.error = err;
    this.priv = null;
    return this;
  };

  ECKey.prototype.setCompressed = v => {
    this.compressed = !!v;
    if (this.pubPoint) this.pubPoint.compressed = this.compressed;
    return this;
  };

  ECKey.prototype.getPub = () => {
    if (this.compressed) {
      if (this.pubComp) return this.pubComp;
      return (this.pubComp = this.getPubPoint().getEncoded(1));
    } else if (this.pubUncomp) return this.pubUncomp;
    return (this.pubUncomp = this.getPubPoint().getEncoded(0));
  };

  ECKey.prototype.getPubPoint = () => {
    if (!this.pubPoint) {
  ...
  ECKey.prototype.getPubKeyHex = () => {
    if (this.compressed) {
      ...
      return (this.pubKeyHexComp = Crypto.util
        ...
        .toString()
        .toUpperCase());
    } else if (this.pubKeyHexUncomp) return this.pubKeyHexUncomp;
  ...
  ECKey.prototype.getPubKeyHash = () => {
    if (this.compressed) {
      if (this.pubKeyHashComp) return this.pubKeyHashComp;
      return (this.pubKeyHashComp = Bitcoin.Util.sha256ripe160(this.getPub()));
    } else if (this.pubKeyHashUncomp) return this.pubKeyHashUncomp;
    return (this.pubKeyHashUncomp = Bitcoin.Util.sha256ripe160(this.getPub()));
  };

  ECKey.prototype.getBitcoinAddress = () => {
    const hash = this.getPubKeyHash();
    const addr = new Bitcoin.Address(hash);
    return addr.toString();
  };

  ECKey.prototype.setPub = pub => {
    // byte array
    if (Bitcoin.Util.isArray(pub)) {
      pub = Crypto.util.bytesToHex(pub).toString().toUpperCase();
    }
    const ecPoint = ecparams.getCurve().decodePointHex(pub);
  ...
  ECKey.prototype.getBitcoinWalletImportFormat = () => {
    let bytes = this.getBitcoinPrivateKeyByteArray();
    if (bytes == null) return '';
    bytes.unshift(ECKey.privateKeyPrefix); // prepend 0x80 byte
    if (this.compressed) bytes.push(0x01); // append 0x01 byte for compressed format
    const checksum = Crypto.SHA256(Crypto.SHA256(bytes, { asBytes: true }), {
      asBytes: true,
    });
    bytes = bytes.concat(checksum.slice(0, 4));
    const privWif = Bitcoin.Base58.encode(bytes);
    return privWif;
  };

  // Private Key Hex Format
  ECKey.prototype.getBitcoinHexFormat = () => {
    return Crypto.util
  ...
  ECKey.prototype.getBitcoinBase64Format = () => {
    return Crypto.util.bytesToBase64(this.getBitcoinPrivateKeyByteArray());
  };

  ECKey.prototype.getBitcoinPrivateKeyByteArray = () => {
  ...
  /**
	...
	*/
  ECKey.decodeCompressedWalletImportFormat = privStr => {
    var bytes = Bitcoin.Base58.decode(privStr);
  ...
  ECKey.isHexFormat = key => {
    key = key.toString();
    return /^[A-Fa-f0-9]{64}$/.test(key);
  };

  // 51 characters base58, always starts with a '5'
  ECKey.isWalletImportFormat = key => {
    key = key.toString();
  ...
  ECKey.isCompressedWalletImportFormat = key => {
    key = key.toString();
    return ECKey.privateKeyPrefix === 0x80
  ...
  ECKey.isBase6Format = key => {
    key = key.toString();
    return /^[012345]{99}$/.test(key);
  };

  // 22, 26 or 30 characters, always starts with an 'S'
  ECKey.isMiniFormat = key => {
    key = key.toString();
    const validChars22 = /^S[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{21}$/.test(
      key
    );
    const validChars26 = /^S[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{25}$/.test(
      key
    );
    const validChars30 = /^S[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{29}$/.test(
      key
    );
    // space between key and ?
    const testBytes = Crypto.SHA256(`${key}?`, { asBytes: true });

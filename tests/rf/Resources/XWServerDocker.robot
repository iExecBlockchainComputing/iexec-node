...
Ping XWtremWeb Database
    MySqlDocker.Xtremweb Tables Must Exist


Clear XWtremWeb Database
    MySqlDocker.Delete All Xtremweb Tables


Count From Datas Where Uid
    [Arguments]  ${uid}  ${countExpected}
    MySqlDocker.Count From Table Where Uid  datas  ${uid}  ${countExpected}

Count From Apps Where Uid
    [Arguments]  ${uid}  ${countExpected}
    MySqlDocker.Count From Table Where Uid  apps  ${uid}  ${countExpected}

Count From Works Where Uid
    [Arguments]  ${uid}  ${countExpected}
    MySqlDocker.Count From Table Where Uid  works  ${uid}  ${countExpected}

Count From Works
    [Arguments]  ${countExpected}
    MySqlDocker.Count From Table  works  ${countExpected}

...
Gradle BuildAll BuildImages Xtremweb
  Run Process  cd ${REPO_DIR}/xtremweb-hep && ./gradlew buildAll buildServerDockerImage buildWorkerDockerImage --refresh-dependencies -Penvironment\=docker  shell=yes stderr=STDOUT  stdout=${REPO_DIR}/xtremweb-hep-build.log
...
Create Robot Chain Json For Scheduler And Deposit Some RLC
    ...
    ${iexec_result} =  Run Process  sudo npm -g i iexec && cd ${REPO_DIR}/xtremweb-hep/build/dist/${dir.stdout}/sdk-dev-accounts/scheduler && iexec account deposit 200 --chain robot  shell=yes  stderr=STDOUT  timeout=340s  stdout=${REPO_DIR}/iexec-sdk.log

...
Test Full V2
    ...
    Wait Until Keyword Succeeds  20 min	3 sec  Check WorkOrderRevealing  @{woid}[0]

    Wait Until Keyword Succeeds  20 min	3 sec  Check WorkOrderCompleted  @{woid}[0]
    ...
    Wait Until Keyword Succeeds  2 min	3 sec  Check Three Marketorder

    # add a 30 seconds Sleep workaround
    Sleep  30 seconds

    ${logs} =  IexecSdk.Iexec An app Docker  order fill 3 --force
    Log  ${logs}
    Should Contain  ${logs}  woid
    @{woid2} =  Get Regexp Matches  ${logs}  woid: '(?P<woid>.*)',  woid
    Log  @{woid2}[0]

    Wait Until Keyword Succeeds  20 min	3 sec  Check WorkOrderRevealing  @{woid2}[0]

    Wait Until Keyword Succeeds  20 min	3 sec  Check WorkOrderCompleted  @{woid2}[0]
    ...
    @{consensus1} =  Get Regexp Matches  ${logs}  "consensus":"(?P<consensus1>.*)","revealDate"  consensus1
    Log  @{consensus1}[0]

    ${logs} =  IexecPocoAPI.Curl On Iexec Poco Api  api/workorders/@{woid2}[0]/consensus
    Log  ${logs}

    @{consensus2} =  Get Regexp Matches  ${logs}  "consensus":"(?P<consensus2>.*)","revealDate"  consensus2
    Log  @{consensus2}[0]
...
Check Three Marketorder
    ${logs} =  IexecPocoAPI.Curl On Iexec Poco Api  api/marketorders/count
    Log  ${logs}
    Should Be Equal As Integers	 ${logs}  3
